...
import org.diediedie.level.actors.statemachine.StateType;
...
public class Elevator extends BaseLevelObject implements SignalReceiver
{
    ...
    final float         DOWN_MAX_SPEED = 5.2f, UP_MAX_SPEED = -4.9f;
    ...
    protected float     speedIncrUp    = 0.1f, speedIncrDown = 1.3f;
    ...
    /*
     * Updates the Elevator. We're not using the StateMachine interface because
     * it is unnecessarily large - the elevator only has two very simple states,
    ...
    /*
     * Sets the current state to IDLE, from ACTIVE. [Elevator has stopped]
     */
    ...
    /*
     * We're overriding this from the BaseLevelObject implementation and doing
     * nothing because the idea of the Elevator class is that it can only be set
     * once.
     */
    @Override
    ...
    /*
     * Elevators can only receive Switch signals from the source parsed from the
